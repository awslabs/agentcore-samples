...
REMEDY_UNPARSEABLE = "Could not parse a principal from the response. Inspect the detail field."
...
def _sse_payloads(body: str):
    ...
    Within one event, consecutive ``data:`` lines are concatenated, per the SSE specification.
    """
...
def run_check(url: str, token: str, target_name: str, tool_name: str, query: str, requested_version: str) -> dict:
    ...
    response = mcp_post(url, token, payload, protocol_version=negotiated)
